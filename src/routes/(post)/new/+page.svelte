<script lang="ts">
	...
	import { conf } from '$lib/tinymce.js';
	...
	const imageRemove = () => {
		coverImageSrc = '';
		let fileVal = <HTMLInputElement>document.getElementById('imageInput');
		fileVal.value = '';
	};
</script>
			...
			<div>
				...
				{:else}
					<Button color="yellow" size="xl" class="mb-6" on:click={imageInputClick}
						>Add a cover image<Icon src={Photo} class="w-6 h-6 ml-2" /></Button
					>
				{/if}
				<input
					type="file"
